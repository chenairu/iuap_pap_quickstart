 ...
 */
@Component
public class ExampleInstitValidator {
    @Autowired
    private ExampleInstitService service;
    

    public void valid(List<ExampleInstit> institList) {
        if (CollectionUtils.isEmpty(institList)) {
            throw new ValidException("提交的数据集为空!");
        }
        StringBuilder builder = new StringBuilder();
        for (ExampleInstit instit : institList) {
