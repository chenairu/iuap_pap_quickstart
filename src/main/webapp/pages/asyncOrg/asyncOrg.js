define([
    ...
    var init = function(element,cookie) {
        ...
        var treelistUrl = ctx + "/asyncOrg/list";
        var treedelUrl = ctx + "/asyncOrg/del/";
        var treesaveUrl = ctx + "/asyncOrg/save";
        ...
        var viewModel = {
            ...
            event: {
                    ...
                    $(element)
                        ...
                        .each(function() {
                            if (
                                ...
                                jsonData["search_searchParam"] = this.value.replace(
                                    /(^\s*)|(\s*$)/g,
                                    ""
                                ); //去掉空格
                            }
                            ...
                            if (res) {
                                if (res.success == "success") {
                                    if (!res.detailMsg.data) {
                                        ...
                                        viewModel.telbookdata.removeAllRows();
                                        viewModel.telbookdata.clear();
                                        viewModel.telbookdata.setSimpleData(
                            ...
                            if (res) {
                                if (res.success == "success") {
                                    if (res.detailMsg.data) {
                                        ...
                                        var arr = viewModel.event.mapKeyAndValues(res.detailMsg.data, true);
                ...
                deleteTree: function(data) {
                    ...
                    datalist = viewModel.event.mapKeyAndValues(datalist, false);
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            if (res) {
                                if (res.success == "success") {
                                    viewModel.institdata.removeRow(
                                        viewModel.institdata.getSelectedIndexs()
                                    );
                                    message("删除成功！");
                                } else {
                                    var msg = "";
                                    if (res.success == "fail_global") {
                                        msg = res.message;
                                        if (
                                            msg == "Data had been referenced,remove is forbidden!"
                                        ) {
                                            msg = "该部门含有子部门不能删除";
                                        }
                                    } else {
                ...
                editinstitClick: function() {
                    ...
                    if (row) {
                        ...
                        viewModel.institdatanew.setSimpleData(
                            viewModel.institdata.getSimpleData({ type: "select" })
                        );
                ...
                editManClick: function(rowId) {
                    ...
                    if (row) {
                        viewModel.telbookdatanew.setSimpleData(
                            row.getSimpleData()
                        );
                    ...
                    if (row) {
                        ...
                        u.confirmDialog({
                            ...
                            onOk: function() {
                                ...
                                viewModel.telbookdata.removeRow(
                                    viewModel.telbookdata.getSelectedIndexs()
                                );
                            },
                            onCancel: function() {}
                        });
            ...
            optFun: function(obj) {
                ...
                // 绑定删除事件
                var delfun = "data-bind=click:event.delManClick.bind($data," + dataTableRowId + ")";
                // 绑定修改事件
                var editfun = "data-bind=click:event.editManClick.bind($data," + dataTableRowId + ")";
                // 绑定查看事件
                obj.element.innerHTML =
                    '<div class="editTable" style="display:none;"><button class="u-button u-button-border" title="修改"' + editfun + ">修改</button>" +
                    '<button class="u-button u-button-border" title="删除" ' + delfun + ">删除</button></div>";
                ko.applyBindings(viewModel, obj.element);
            },
            //删除操作
            del: function(rowId) {
                var datatableRow = viewModel.telbookdata.getRowByRowId(rowId);
                //请求后端删除对应的数据；
                // index为数据下标
                u.confirmDialog({
                    msg:
                    ...
                    title: "",
                    onOk: function() {
                        viewModel.telbookdata.removeRow(datatableRow);
                    }
                });
            },
        };
